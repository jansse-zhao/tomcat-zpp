...
public class StandardContext extends ContainerBase implements Context, NotificationEmitter {
    ...
    /**
     * Context组件是Web应用的抽象，我们开发的Web应用部署Tomcat后运行时就会转化成Context对象，
     * 它包含了各种静态资源、若干Servlet（Wrapper容器）以及各种其他动态资源。
     * <p>
     * 包含如下组件：
     * Listener 组件:可以在Tomcat生命周期中完成某些Context 容器相关工作的监听器
     * AccessLog 组件:客户端的访问日志，对该Web应用的访问都会被记录。
     * Pipeline 组件:Context容器对请求进行处理的管道。
     * Realm 组件:提供了 Context 容器级别的用户-密码-权限的数据对象，配合资源认证模块使用。
     * Loader 组件:Web应用加载器，用于加载Web应用的资源，它要保证不同 Web 应用之间的资源隔离。
     * Manager 组件:会话管理器，用于管理对应Web容器的会话，包括维护会话的生成、更新和销毁。
     * NamingResource 组件:命名资源，它负责将Tomcat配置文件的server.xml和 Web应用的context.xml资源和属性映射到内存中。
     * Mapper 组件:Servlet映射器，它属于Context内部的路由映射器，只负责该Context 容器的路由导航。
     * Wrapper 组件:Context 的子容器。
     */
    ...
    /**
     * Lifecycle provider.
     * 实例管理器，它主要的作用就是实现对Context 容器中监听器过滤器以及 Servlet等实例的管理。
     * 它的实现其实很简单，其中就使用了一些反射机制实例化出对象，但这里面需要注意的地方是，InstanceManager包含了两个类加载器，
     * 一个是属于Tomcat 容器内部的类加载器，而另外一个是 Web 应用的类加载器。Tomcat 容器类加载器是 Web 应用类加载器的父类加载器，
     * 且 Tomcat 容器类加载器在 Tomcat 的整个生命周期中都存在，而 Web 应用类加载器则不同，它可能在重启后则被丢弃，最终被GC回收。
     */
     ...
     * <p>
     * 过滤器对应的web.xml文件中的filter-mapping和filter-name和url-pattern
     */
     ...
     * NamingResources组件负责的工作是将配置文件中声明的不同的资源及其属性映射到内存中，这些映射统一由NamingResources对象封装
     */
